...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:sqf="http://www.schematron-quickfix.com/validator/process" queryBinding="xslt2">
    ...
    <pattern>
        <rule context="child-of[. ne ''] | predecessor | successor">
            <assert test="doc-available(concat('../data/units/current-units/', ., '.xml')) or doc-available(concat('../data/units/discontinued-units/', ., '.xml')) or doc-available(concat('../data/units/predecessor-units/', ., '.xml'))">No matching office ID found for "<value-of select="."/>" in units
                folder</assert>
        </rule>
    </pattern>
    <pattern>
